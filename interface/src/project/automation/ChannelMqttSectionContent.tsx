...
import { RestFormProps, FormActions, FormButton, SectionContent } from '../../components';

import { ChannelMqttSettings } from './types';

type ChannelMqttSettingsControllerFormProps = RestFormProps<ChannelMqttSettings>;

function ChannelMqttSectionContent(props: ChannelMqttSettingsControllerFormProps) {
  const { data, saveData, loadData, handleValueChange } = props;
  return (
    <ValidatorForm onSubmit={saveData}>
      <Box bgcolor="primary.main" color="primary.contrastText" p={2} mt={2} mb={2}>
        <Typography variant="body1">
          The Switch is controllable via MQTT designed to work with Home Assistant's auto discovery feature.
        </Typography>
      </Box>
      <TextValidator
        validators={['required']}
...
export default ChannelMqttSectionContent;